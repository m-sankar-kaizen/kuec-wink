...
class WinkRequest(http.Controller):
    ...
    def register_and_request(self, **post):
        ...
        template = request.env.ref('kuec_service_catalogue.kuec_portal_welcome_email_v3', raise_if_not_found=False)
        if template:
    ...
    def submit_request(self, **post):
        ...
        template = request.env.ref('kuec_service_catalogue.kuec_coordinator_notification_email_v3', raise_if_not_found=False)
        if template:
