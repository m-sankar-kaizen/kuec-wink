...
class PropertyReturnDate(models.TransientModel):
    ...
    def validate_return_date(self):
        """The function used to validate the return date"""
        custody_id = self._context.get('custody_id')
        if custody_id:
            custody_obj = self.env['hr.custody'].browse(custody_id)
            if custody_obj and custody_obj.date_request:
                if self.returned_date <= custody_obj.date_request:
                    raise ValidationError('Please give a valid renewal date.')
