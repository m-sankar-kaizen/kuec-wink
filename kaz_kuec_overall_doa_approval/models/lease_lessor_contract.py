...
class LeaseLessorContract(models.Model):
    ...
    _inherit = ['lease.lessor.contract', 'approval.base.mixin']
    ...
    lessor_contract_approval_ids = fields.One2many('lease.lessor.contract.approval',
                                                   'lessor_contract_id',
                                                   string='Approval/Rejection/Return History')

    @property
    def _approval_model(self):
        return 'lease.lessor.contract.approval'

    @property
    def _approval_line(self):
        return self.lessor_contract_approval_ids

    def _approval_context(self):
        self.ensure_one()
        return {
            'default_lessor_contract_id': self.id
        }
    ...
    def submit_for_approval(self):
        ...
        summary = _('Approval of Lessor Contract'),
        note = _('Please approve the request for Lessor Contract')
    ...
    def action_hod_approval(self):
        ...
        if self._context.get('signed', False):
            summary = _("Lessor Contract Request Requires Your Review")
            note = _(
                f"The Lessor Contract Request has been Requested by {self.env.user.display_name} "
                "Please review the submitted Lessor Contract Request and proceed with the next steps."
            )
    ...
    def action_reject_request(self):
        self.ensure_one()
        if self._context.get('signed', False):
            summary = _("Lessor Contract Form Rejected")
            note = _(
                f"The Lessor Contract request has been rejected by {self.env.user.display_name}. "
                "Please review the submitted Lessor Contract form"
            )
            self._perform_action('rejected', self.create_uid, summary, note)
            self.action_cancel()
            return True
        else:
            return self._open_approve_reject_wizard('Reject Lessor Contract', 'reject',
                                                    'action_reject_request')

    def action_rfc_request(self):
        self.ensure_one()
        if self._context.get('signed', False):
            summary = _("Lessor Contract Form Returned for Correction")
            note = _(
                f"The Lessor Contract request has been Returned for Correction by {self.env.user.display_name}. "
                "Please review the submitted Lessor Contract form"
            )
