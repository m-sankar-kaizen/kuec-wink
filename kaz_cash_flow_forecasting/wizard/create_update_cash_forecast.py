...
class CreateUpdateCashForecast(models.TransientModel):
    ...
    cash_forecast_type_ids = fields.Many2many('kaz.cash.forecast.type',
    ...
    def create_manual_cash_forecast(self, period_id):
        cash_forecast_obj = self.env['kaz.cash.forecast']
        cash_forecast_type_ids = self.cash_forecast_type_ids
        if period_id.is_locked:
            raise ValidationError(_("Selected period %s is locked; generation is not allowed.") % (period_id.code,))
        if not cash_forecast_type_ids:
            cash_forecast_type_ids = self.env['kaz.cash.forecast.type'].search(
        ...
        for cash_forecast_type_id in cash_forecast_type_ids.sorted('sequence'):
            if cash_forecast_type_id.approve_forecast_type(period_id):
                ...
                is_previously_created = self.env['kaz.cash.forecast'].search(
                    [('forecast_period_id', '=', period_id.id),
                     ('forecast_type_id', '=', cash_forecast_type_id.id),
                     ('company_id', '=', cash_forecast_type_id.company_id.id),
    ...
    def create_cash_forecast(self):
        ...
        if self.period_id and self.period_id.is_locked:
            raise ValidationError(
                _("Selected period %s is locked; generation is not allowed.") % (self.period_id.code,))
        ...
        cash_forecast = self.env['kaz.cash.forecast'].search([('forecast_type_id', 'in', cash_forecast_type_ids.ids),
    ...
    def calculate_value(self):
        if self.calculate == 'forecast':
            ...
            if not self.period_id or self.period_id.start_date <= month_start_date:
                self.env['kaz.cash.forecast'].get_real_values(self.period_id.id)
            else:
                raise ValidationError("please select current or past period")
        self.env['onboarding.onboarding.step'].sudo().action_validate_step(
            'kaz_cash_flow_forecasting.onboarding_onboarding_step_create_forecast')
        return {
            'name': _('Cash Forecast'),
            'view_mode': 'pivot,list,form',
            'res_model': 'kaz.cash.forecast',
            'type': 'ir.actions.act_window',
        }

    def update_cash_forecast(self):
        action = self.env.ref('kaz_cash_flow_forecasting.actions_create_update_cash_forecast').sudo().read()[0]
