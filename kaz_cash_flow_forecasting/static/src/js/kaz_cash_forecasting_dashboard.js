...
export class kaz_cash_flow_forecasting_dashboard extends Component {
    setup() {
        super.setup();
        ...
        await ensureJQuery();
            Object.assign(this.dashboardData, await this.orm.call(
                'kaz.cash.flow.forecasting.dashboard',
                'get_dashboard_data',
                [], {
                    'context': context
                }));
    ...
    on_attach_callback() {
        ...
        debugger
            self.kaz_charts('expanse', 'bar', 'true');
        });
        $('#switchLine').change(function() {
            self.kaz_charts('expanse', 'line', 'true');
        });
        $('#switchIncomeBar').change(function() {
            self.kaz_charts('income', 'bar', 'true');
        });
        $('#switchIncomeLine').change(function() {
            self.kaz_charts('income', 'line', 'true');
        });
        ...
        this.kaz_charts('expanse', 'bar', 'false');
        this.kaz_charts('income', 'bar', 'false');
        this.income_vs_expense_value_chart('income_vs_expense_value');

        self.filterFiscalPeriod();
    }
    kaz_charts(chart_id, chartType, isFilter) {
    ...
    on_click_option(ev) {
        ...
        if (option.html() != $(selectBtn).html()) {
            if (option[0].innerText != "Custom Fiscal Period") {
                ...
                this.orm.call('kaz.cash.flow.forecasting.dashboard', 'get_dashboard_data', [
                    [{
                    ...
                    if ($(option).hasClass('option-cashout')) {
                        self.kaz_charts('expanse', 'bar', 'false');
                    }
                    if ($(option).hasClass('option-cashin')) {
                        self.kaz_charts('income', 'bar', 'false');
                        //self.kaz_charts('expanse','bar','false');
                    }
    ...
    filterFiscalPeriod() {
        ...
        $('#apply_filter').on('click', function() {
            ...
            if (EndDate.options[EndDate.selectedIndex] == null && StartDate.options[StartDate.selectedIndex] == null) {
                ...
                self.orm.call('kaz.cash.flow.forecasting.dashboard', 'get_dashboard_data', [
                    [{
                        "filter": "time_period"
                    }, {
...
kaz_cash_flow_forecasting_dashboard.template = 'kazCashForecastDashboard';

registry.category('actions').add('kaz_cash_flow_forecasting_dashboard', kaz_cash_flow_forecasting_dashboard);
