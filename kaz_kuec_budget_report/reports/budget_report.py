...
#         super()._compute_reserved_amount()
...
class BudgetReport(models.Model):
    ...
    manpower_committed_amount = fields.Float(
        string="Manpower Committed Amount",
        group_operator='sum',
        readonly=True
    )
    total_committed_amount = fields.Float(
        string="Total Committed Amount",
        group_operator='sum',
        readonly=True
    )
    ...
    def _get_bl_query(self, plan_fnames):
        return SQL(
            ...
            SELECT CONCAT('bl', bl.id::TEXT) AS id,
                   ...
                   bl.general_budget_id AS general_budget_id,
                   bl.budget_line_tag_id AS budget_line_tag_id,
                   bl.master_plan_id AS master_plan_id,
                   NULL AS user_id,
                   'budget' AS line_type,
                   bl.budget_amount AS budget,
                   0 AS committed,
                   0 AS achieved,
                   0 AS manpower_committed_amount,
                   0 AS total_committed_amount,
                   %(plan_fields)s
              FROM budget_line bl
              JOIN budget_analytic ba ON ba.id = bl.budget_analytic_id
            """,
            plan_fields=SQL(', ').join(
                self.env['budget.line']._field_to_sql('bl', fname)
    ...
    def _get_aal_query(self, plan_fnames):
        return SQL(
            ...
            SELECT CONCAT('aal', aal.id::TEXT) AS id,
                   ...
                   NULL::INTEGER AS general_budget_id,
                   NULL::INTEGER AS budget_line_tag_id,
                   NULL::INTEGER AS master_plan_id,
         ...
         LEFT JOIN budget_analytic ba ON ba.id = bl.budget_analytic_id
            ...
            """,
            analytic_fields=SQL(', ').join(
                self.env['account.analytic.line']._field_to_sql('aal', fname)
                for fname in plan_fnames
            ),
    ...
    def _get_pol_query(self, plan_fnames):
        ...
        return SQL(
            ...
            SELECT (pol.id::TEXT || '-' || ROW_NUMBER() OVER (PARTITION BY pol.id ORDER BY pol.id)) AS id,
                   ...
                   0 AS achieved,
                   COALESCE(bl.manpower_committed_amount, 0) AS manpower_committed_amount,
                   COALESCE(bl.total_committed_amount, 0) AS total_committed_amount,
                   %(analytic_fields)s
        ...
        CROSS JOIN JSONB_TO_RECORDSET(pol.analytic_json) AS a(rate FLOAT, %(field_cast)s)
         ...
         LEFT JOIN budget_analytic ba ON ba.id = bl.budget_analytic_id
            ...
            """,
            analytic_fields=SQL(', ').join(
                self.env['account.analytic.line']._field_to_sql('a', fname)
                for fname in plan_fnames
            ),
            qty_invoiced_table=qty_invoiced_table,
            field_cast=SQL(', ').join(
                SQL('%s FLOAT', SQL.identifier(fname)) for fname in plan_fnames
            ),
            condition=SQL(' AND ').join(SQL(
