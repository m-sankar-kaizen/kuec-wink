...
from odoo.addons.kaz_advanced_web_domain_widget.models.domain_prepare import prepare_domain_v2,compute_domain
...
class ir_rule(models.Model):
    ...
    def _compute_domain(self, model_name, mode="read"):
        ...
        read_value = True
        self._cr.execute("SELECT state FROM ir_module_module WHERE name='kaz_kuec_simplify_access_management'")
        ...
        if not value:
            value = self._cr.execute("""select state from ir_module_module where name = 'kaz_kuec_simplify_access_management'""")
            value = self._cr.fetchone()
