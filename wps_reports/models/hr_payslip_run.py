...
class HrPayslipRun(models.Model):
    """
        Inherits: hr.payslip.run

        Extends the payroll batch to generate various customized payroll Excel reports, including:
        - WPS Report
        - Bank Transfer Reports (BT and LBT)
        - Payroll Register
        - Housing Advance Report

        Also provides utility method to calculate unpaid time-off for an employee within the payslip batch period.
        """
    _inherit = 'hr.payslip.run'

    def get_employee_unpaid_timeoff(self, emp_id):
        """
               Computes the total number of unpaid leave days for a given employee within the payslip run period.

               :param emp_id: ID of the employee.
               :return: Total number of unpaid leave days (float).
               """
        unpaid_time_off = self.env.ref("hr_holidays.holiday_status_unpaid")
        if unpaid_time_off:
            leaves = self.env['hr.leave'].sudo().search(
                [('holiday_status_id', '=', unpaid_time_off.id),
                 ('state', '=', 'validate'),
                 ('employee_id', 'in', [emp_id]), ('active_employee', '=', True)])
            num_of_days = 0
            if leaves:
                for leave in leaves:
                    if leave.request_date_from > self.date_start and leave.request_date_to < self.date_end:
                        num_of_days += leave.number_of_days
                    if leave.request_date_from > self.date_start and leave.request_date_to > self.date_end:
                        num_of_days += (self.date_end - leave.request_date_from).days
                    if leave.request_date_from < self.date_start and leave.request_date_to < self.date_end:
                        num_of_days += (leave.request_date_to - self.date_start).days
                    if leave.request_date_from < self.date_start and leave.request_date_to > self.date_end:
                        num_of_days += (self.date_end - self.date_start).days
            return num_of_days
    ...
    def print_registration_report(self):
        ...
        header_style = {
            ...
            "fill": PatternFill(start_color="983513", end_color="983513", fill_type="solid"),
            "border": Border(
    ...
    def print_housing_advance(self):
        """
                Generates a Housing Advance Excel report showing loan/advance details per employee.
                Includes fields like Person Extra Info ID, KU Number, Loan Amount, Installments, and Creation Date.
                """
        workbook = openpyxl.Workbook()
        worksheet = workbook.active
        worksheet.title = 'Housing Advance Report'

        column_widths = [30, 30, 30, 30, 30, 30, 30, 30, 30]
        for i, width in enumerate(column_widths, start=1):
            worksheet.column_dimensions[get_column_letter(i)].width = width

        headers = [
            "Person Extra Info ID", "Employee #", "Full Name",
            "Assignment Category", "Payroll Name", "Reference", "Loan Amount",
        ...
        string = f"Housing_{month}_{year}"
        cell1 = worksheet.cell(row=1, column=1,
                               value="Housing Advance Report - " + string + " - " + self.name)
        ...
        header_style = xlwt.easyxf(
            'font: bold on; pattern: pattern solid, fore_colour ocean_blue; align: horiz center;')
        for col_num, header in enumerate(headers):
            cell = worksheet.cell(row=2, column=col_num + 1, value=header)
            cell.fill = header_fill
            cell.font = header_font
        ...
        if self.slip_ids:
            for slip in self.slip_ids:
                ...
                if slip.contract_id and slip.employee_id:
                    total += slip.contract_id.loan_amount
                    worksheet.cell(row, col, slip.contract_id.person_extra_info or "")
                    col += 1

                    worksheet.cell(row, col, slip.employee_id.ku_number or "")
                    col += 1

                    worksheet.cell(row, col, slip.employee_id.display_name or "")
                    col += 1
                    worksheet.cell(row, col,
                                   slip.contract_id.assignment_category_id.name if slip.contract_id.assignment_category_id else "")
                    col += 1
                    ...
                    worksheet.cell(row, col, slip.contract_id.reference or "")
                    col += 1
                    worksheet.cell(row, col, slip.contract_id.loan_amount or 0)
                    col += 1

                    worksheet.cell(row, col, slip.contract_id.installment or "")
                    col += 1

                    worksheet.cell(row, col, slip.contract_id.creation_date or "")
                    col = 1

                    row += 1
        worksheet.cell(row, col + 5, "Total")
        worksheet.cell(row, col + 6, total)

        report_file = BytesIO()
        workbook.save(report_file)
        report_file.seek(0)

        filename = 'Housing Advance ' + self.date_end.strftime("%B %Y") + ".xlsx"
        attachment = self.env['ir.attachment'].create({
            'name': filename,
            'datas': base64.encodebytes(report_file.read()),
            'res_model': self._name,
