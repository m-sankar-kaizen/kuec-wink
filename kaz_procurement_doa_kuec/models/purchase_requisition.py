...
class PurchaseRequisition(models.Model):
    ...
    _inherit = ['approval.base.mixin', 'purchase.requisition']
    ...
    purchase_requisition_approval_ids = fields.One2many(
        'purchase.requisition.approval', 'requisition_id',
    ...
    def _compute_amount_total(self):
        for rec in self:
            rec.amount_total = sum([line.product_qty * line.price_unit for line in rec.line_ids])
    ...
    def _approval_line(self):
        return self.purchase_requisition_approval_ids
    ...
    def action_lock_pa(self):
        self.ensure_one()
        self.is_readonly = True

    def action_unlock_pa(self):
        self.ensure_one()
    ...
    def _perform_common_action(self, state, group_xml_id):
        summary = _("Purchase Agreement Request Requires Your Review")
        note = _(
            f"The Purchase Agreement Request has been approved by {self.env.user.display_name} "
            "Please review the submitted Purchase Agreement Request form and proceed with the next steps."
        )
    ...
    def submit_to_approve(self):
        if self._check_company_access():
            ...
            summary = _("Purchase Agreement Request Requires Your Review")
            note = _(
                f"The Purchase Agreement Request has been Requested by {self.env.user.display_name} "
                "Please review the submitted Purchase Agreement Request form and proceed with the next steps."
            )
    ...
    def action_reject_request(self):
        self.ensure_one()
        if self._context.get('signed', False):
            summary = _("Purchase Agreement Request Rejected")
            note = _(
                f"The Purchase Agreement Request has been rejected by {self.env.user.display_name}. "
                "Please review the submitted Purchase Agreement Request"
            )
            self._perform_action('rejected', self.create_uid, summary, note)
            self.state = 'cancel'
            return True
        else:
            return self._open_approve_reject_wizard('Reject Purchase Agreement', 'reject',
                                                    'action_reject_request')

    def action_rfc_request(self):
        self.ensure_one()
        if self._context.get('signed', False):
            summary = _("Purchase Agreement Request Returned for Correction")
            note = _(
                f"The Purchase Agreement Request has been Returned for Correction by {self.env.user.display_name}. "
                "Please review the submitted Purchase Agreement Request form"
            )
            self._perform_action('draft', self.create_uid, summary, note)
            self.state = 'draft'
            self.action_unlock_pa()
            return True
        else:
